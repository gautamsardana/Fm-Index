...
void build_rank(JacobsonRank &rank, const std::vector<uint8_t> &bwt,
  ...
  uint64_t total_sub_chunks = num_chunks * rank.sub_chunks_per_chunk;
  uint64_t total_bits = total_sub_chunks * rank.bits_per_relative_rank;
...
uint64_t get_rank(const JacobsonRank &rank, uint64_t offset) {
  ...
  uint64_t bit_pos    = global_sub * rank.bits_per_relative_rank;
  uint64_t relative   = get_bits(rank.relative_ranks, bit_pos, rank.bits_per_relative_rank);

  uint64_t num_remaining = std::min(offset, rank.n) - sub_start;
  uint64_t remaining = (num_remaining > 0) ? popcount(get_bits(bwt, sub_start, num_remaining)) : 0;
